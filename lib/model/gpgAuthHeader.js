...
GpgAuthHeader.prototype.validate = function(step) {
  ...
  if(result instanceof Error) {
    error_msg = __('The server was unable to respect the authentication protocol.');
  ...
  switch(stage) {
    ...
    case 'stage0' :
      ...
      if(typeof this.headers['x-gpgauth-user-auth-token'] !== 'undefined') {
        return new Error(__('x-gpgauth-user-auth-token should not be set during the verify stage' +
          typeof this.headers['x-gpgauth-user-auth-token']));
      }
    ...
    case 'stage2' :
      if(typeof this.headers['x-gpgauth-authenticated'] !== 'string' ||
        this.headers['x-gpgauth-authenticated'] != 'false') {
        return new Error(__('x-gpgauth-authenticated should be set to false during stage2'));
      }
      if(typeof this.headers['x-gpgauth-progress'] !== 'string' ||
        this.headers['x-gpgauth-progress'] != 'stage2') {
        return new Error(__('x-gpgauth-progress should be set to stage2'));
      }
      if(typeof this.headers['x-gpgauth-user-auth-token'] !== 'undefined') {
        return new Error(__('x-gpgauth-user-auth-token should not be set during stage2'));
      }
      if(typeof this.headers['x-gpgauth-verify-response'] !== 'undefined') {
        return new Error(__('x-gpgauth-verify-response should not be set during stage2'));
      }
      if(typeof this.headers['x-gpgauth-refer'] !== 'undefined') {
        return new Error(__('x-gpgauth-refer should not be set during stage2'));
      }
      return true;
