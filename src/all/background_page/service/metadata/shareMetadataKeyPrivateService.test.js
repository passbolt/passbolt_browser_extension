...
import {
  metadataKeysNotSignedByCurrentDto,
...
describe("ShareMetadataKeyPrivateService", () => {
  ...
  describe("::shareOneMissing", () => {
    ...
    it("should assert passphrase parameter.", async () => {
      ...
      await expect(() => service.shareOneMissing(pgpKeys.betty.userId, 2)).rejects.toThrow(
    ...
    it("should throw if user is not an administrator.", async () => {
      ...
      await expect(() => service.shareOneMissing(pgpKeys.betty.userId, pgpKeys.ada.passphrase)).rejects.toThrow(
    ...
    it("should share and sign the missing data keys signed by the current administrator", async () => {
      ...
      jest
        .spyOn(service.metadataPrivateKeyApiService, "create")
        .mockImplementationOnce(async (encryptedMetadataPrivateKeys) => {
      ...
      jest
        ...
        .mockImplementationOnce(async (encryptedMetadataPrivateKeys) => {
          ...
          const decryptResult = await DecryptMessageService.decrypt(
            ...
            recipientPrivateKey,
            [decryptedPrivateKey],
            { returnOnlyData: false, throwOnInvalidSignaturesVerification: false },
          );
          const signature = await FindSignatureService.findSignatureForGpgKey(
            decryptResult.signatures,
            decryptedPrivateKey,
  ...
  describe("::shareAllMissing", () => {
    ...
    it("should throw if user is not an administrator.", async () => {
      ...
      await expect(() => service.shareAllMissing(pgpKeys.betty.userId, pgpKeys.ada.passphrase)).rejects.toThrow(
        "This action can only be performed by an administrator.",
      );
    });
