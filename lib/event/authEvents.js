...
var listen = function (worker) {
    ...
    // Listen to the request to check if the public registration is open.
    worker.port.on('passbolt.auth.isPublicRegistrationOpen', function (token, domain) {
        auth.isPublicRegistrationOpen(domain).then(
            function success(msg) {
                worker.port.emit('passbolt.auth.isPublicRegistrationOpen.complete', token, 'SUCCESS', msg);
            },
            function error(msg) {
                worker.port.emit('passbolt.auth.isPublicRegistrationOpen.complete', token, 'ERROR', msg);
            }
        );
    });
