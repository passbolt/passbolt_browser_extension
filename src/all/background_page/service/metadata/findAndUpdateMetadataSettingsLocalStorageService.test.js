...
describe("FindAndUpdateMetadataSettingsLocalStorageService", () => {
  ...
  beforeEach(async () => {
    ...
    findAndUpdateMetadataSettingsService = new FindAndUpdateMetadataSettingsLocalStorageService(
      account,
      apiClientOptions,
    );
  ...
  describe("::findAndUpdateTypesSettings", () => {
    it("retrieves the metadata types settings from the API and store them into the local storage.", async () => {
      ...
      jest
        .spyOn(
          findAndUpdateMetadataSettingsService.findMetadataSettingsService.metadataTypesSettingsApiService,
          "findSettings",
        )
    ...
    it("with v4, retrieves default v4 metadata types settings without calling the API and store them into the local storage.", async () => {
      ...
      const expectedMetadataTypesSettingsDto = defaultMetadataTypesSettingsV4Dto();
      expect(
        findAndUpdateMetadataSettingsService.findMetadataSettingsService.metadataTypesSettingsApiService.findSettings,
  ...
  describe("::findAndUpdateKeysSettings", () => {
    it("retrieves the metadata keys settings from the API and store them into the local storage.", async () => {
      ...
      jest
        .spyOn(
          findAndUpdateMetadataSettingsService.findMetadataSettingsService.metadataKeysSettingsApiService,
          "findSettings",
        )
    ...
    it("overrides local storage with a second update call.", async () => {
      ...
      jest
        ...
        .mockImplementation(() => metadataKeysSettingsDto);
      await findAndUpdateMetadataSettingsService.metadataKeysSettingsLocalStorage.set(
    ...
    it("waits any on-going call to the update and returns the result of the local storage.", async () => {
      ...
      expect(
        findAndUpdateMetadataSettingsService.findMetadataSettingsService.metadataKeysSettingsApiService.findSettings,
      ).toHaveBeenCalledTimes(1);
