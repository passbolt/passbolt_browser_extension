    ...
    var deferred = defer(),
        ...
        _loop = function (masterPassword) {
            ...
            if (typeof masterPassword == 'undefined') {
                ...
                Worker.get('App', worker).port.emit('passbolt.keyring.master.request', token);
            }
            else {
                // Only up to 3 attempts are authorized
                if (attempts > 2) {
                    deferred.reject();
                    Worker.get('MasterPassword', worker).port.emit('passbolt.keyring.master.request.complete', token, 'ERROR', attempts);
                    return;
                }
                ...
                keyring.checkPassphrase(masterPassword).then(
                    function() {
                      // If everything went fine close the passphrase popup.
                        Worker.get('App', worker).port.emit('passbolt.keyring.master.request.close', token);
