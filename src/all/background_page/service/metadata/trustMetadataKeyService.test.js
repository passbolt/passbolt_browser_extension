...
describe("TrustMetadataKeyService", () => {
  ...
  describe("::trust", () => {
    it("if private metadata key data not already signed: sign the metadata private key data, update the private key on the API, update the metadata key in the session storage, and pin the key.", async () => {
      ...
      jest
        .spyOn(service.updateMetadataKeyPrivateService.metadataPrivateKeyApiService, "update")
        .mockImplementationOnce(() => updatedMetadataPrivateKeyDto);
      ...
      expect(metadataKeyDtosInSessionStorage[0].metadata_private_keys[0].data_signed_by_current_user).toStrictEqual(
        trustedMetadataKey.signed,
      );
      expect(metadataKeyDtosInSessionStorage[0].metadata_private_keys[0].modified).toStrictEqual(
        updatedMetadataPrivateKeyDto.modified,
      );
    ...
    it("throws if the metadata private key parameter is not valid.", async () => {
      expect.assertions(1);
      await expect(() => service.trust({})).rejects.toThrow(
    ...
    it("throws if the passphrase parameter is not valid.", async () => {
      ...
      await expect(() => service.trust(metadataPrivateKey, 42)).rejects.toThrow(
        "The `passphrase` parameter should be a string.",
      );
    });
    ...
    it("throws if the metadata private key data is encrypted.", async () => {
      ...
      await expect(() => service.trust(metadataPrivateKey, "passphrase")).rejects.toThrow(
