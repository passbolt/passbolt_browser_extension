...
pageMods.keysPreferences = pageMod.PageMod({
  include: 'about:blank?passbolt=keysPreferences*',
  contentStyleFile: [
    data.url("css/main_ff.css")
  ],
  contentScriptFile: [
    ...
    data.url('js/inc/keyring.js'),
    data.url("js/inc/event.js"),
    data.url('js/inc/preferences/keys.js')
  ],
  contentScriptWhen: 'ready',
  contentScriptOptions: {
    expose_messaging: false,
    addonDataPath: data.url(),
    templatePath: './tpl/preferences/keys.ejs'
  },
  onAttach: listenPreferencesEvents
});
...
var masterPasswordRequiredFor = function(func, token) {
  var deferred = defer(),
    ...
    _loop = function(masterPassword) {
      ...
      // Try to launch the function given in parameter with the master password given by the password.
...
function listenPreferencesEvents(worker) {
  workers['Preferences'] = worker;
  includeTemplateListeners(worker);
...
function listenAppEvents(worker) {
  ...
  console.log("listen app events on ", Config.read('baseUrl'));
