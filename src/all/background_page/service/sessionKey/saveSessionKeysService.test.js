...
import { sharedResourcesSessionKeys } from "passbolt-styleguide/src/shared/models/entity/sessionKey/sessionKeysCollection.test.data";
...
describe("SaveSessionKeysService", () => {
  ...
  describe("::save", () => {
    it("throws if parameters are not valid.", async () => {
      expect.assertions(5);
      await expect(() => saveSessionKeysService.save()).rejects.toThrow(
        'The parameter "sessionKeys" should be a SessionKeysCollection.',
      );
      await expect(() => saveSessionKeysService.save(42)).rejects.toThrow(
        'The parameter "sessionKeys" should be a SessionKeysCollection.',
      );
      await expect(() => saveSessionKeysService.save({})).rejects.toThrow(
        'The parameter "sessionKeys" should be a SessionKeysCollection.',
      ...
      await expect(() => saveSessionKeysService.save(sessionKeys, 42)).rejects.toThrow(
        'The parameter "passphrase" should be a string.',
      );
      const passphrase = pgpKeys.admin.passphrase;
      await expect(() => saveSessionKeysService.save(sessionKeys, passphrase, 42)).rejects.toThrow(
        'The parameter "retryUpdate" should be a boolean.',
      );
    });
    ...
    it("save a new session keys bundle when none were yet persisted.", async () => {
      ...
      jest
        .spyOn(saveSessionKeysService.getOrFindSessionKeysService, "getOrFindAllBundles")
        .mockImplementationOnce(() => new SessionKeysBundlesCollection());
      ...
      const expectedSerializedSessionKeysBundleData = JSON.stringify({
        object_type: "PASSBOLT_SESSION_KEYS",
        session_keys: sessionKeysDto,
      });
      await expect(createRequestData.data).toDecryptAndEqualTo(
        pgpKeys.ada.private_decrypted,
        expectedSerializedSessionKeysBundleData,
        pgpKeys.ada.private_decrypted,
      );
    });
    ...
    it("update the existing keys bundle with the new session keys.", async () => {
      ...
      jest
        .spyOn(saveSessionKeysService.getOrFindSessionKeysService, "getOrFindAllBundles")
        .mockImplementationOnce(() => existingSessionKeysBundles);
      ...
      const expectedSerializedSessionKeysBundleData = JSON.stringify({
        object_type: "PASSBOLT_SESSION_KEYS",
        session_keys: newSessionKeysDto,
      });
      await expect(createRequestData.data).toDecryptAndEqualTo(
        pgpKeys.ada.private_decrypted,
        expectedSerializedSessionKeysBundleData,
    ...
    it("update the most recent session keys bundle with the new session keys and delete the other ones.", async () => {
      ...
      const existingSessionKeysBundleDto2 = decryptedSessionKeysBundleDto({
        user_id: existingSessionKeysBundleDto1.user_id,
        modified: "2021-10-11T08:09:00+00:00",
      });
      ...
      const expectedSessionKeysBundleDataDto = {
        object_type: "PASSBOLT_SESSION_KEYS",
        session_keys: newSessionKeysDto,
      };
      ...
      expect(saveSessionKeysService.sessionKeysBundleApiService.delete).toHaveBeenCalledWith(
        existingSessionKeysBundleDto2.id,
      );
      expect(saveSessionKeysService.sessionKeysBundlesSessionStorageService.set).toHaveBeenCalledWith(
        new SessionKeysBundlesCollection([
          {
            ...createRequestData.toDto(),
            data: expectedSessionKeysBundleDataDto,
            modified: "2024-11-20T22:30:00+00:00",
          },
        ]),
      );
    });
