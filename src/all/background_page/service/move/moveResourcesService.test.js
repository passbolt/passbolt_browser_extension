...
describe("MoveResourcesService", () => {
  ...
  describe("should assert data", () => {
    it("should assert that the resourceIds are not empty", async () => {
      expect.assertions(1);
      await expect(() => service.moveAll([], "")).rejects.toThrow(
        "Could not move, expecting at least a resource to be provided.",
      );
    });

    it("should assert that the resourceIds is all Uuids", async () => {
      expect.assertions(1);
      await expect(() => service.moveAll([uuidv4(), 42], "")).rejects.toThrow(
        "Could not move, expecting resourcesIds to be an array of UUIDs.",
      );
    });
    ...
    it("should assert that the destinationFolderId is a valid UUID", async () => {
      expect.assertions(1);
      await expect(() => service.moveAll([uuidv4()], "42")).rejects.toThrow(
        "Could not move, expecting destinationFolderId to be a valid UUID.",
      );
    });

    it("should assert that the passphrase is a valid string", async () => {
      expect.assertions(1);
      await expect(() => service.moveAll([uuidv4()], uuidv4(), 42)).rejects.toThrow(
    ...
    it("should assert that the destination folder ID is existing", async () => {
      ...
      await expect(() => service.moveAll([uuidv4()], destinationFolderId)).rejects.toThrow(
        `Could not move, the destination folder does not exist.`,
      );
    ...
    it("should assert that all the resources ids exist", async () => {
      ...
      jest
        .spyOn(service.findFoldersService, "findByIdWithPermissions")
        .mockImplementation(() => new FolderEntity(defaultFolderDto()));
      jest
        .spyOn(service.findResourcesService, "findAllByIdsWithPermissions")
        .mockImplementation(() => new ResourcesCollection([defaultResourceDto({ id: resourceIds[0].id })]));

      await expect(() => service.moveAll(resourceIds, uuidv4())).rejects.toThrow(
        `Could not move, some resources do not exist.`,
      );
  ...
  describe("should move the resources according to the specifications", () => {
    it("moves and updates permissions with: owner resources, in owner or update folders, to private folder", async () => {
      ...
      jest
        ...
        .mockImplementation(
          (ids) => new FoldersCollection(foldersDto.filter((folderDto) => ids.includes(folderDto.id))),
        );
      jest
        .spyOn(service.findFoldersService, "findByIdWithPermissions")
        .mockImplementation((id) => new FolderEntity(foldersDto.find((folderDto) => folderDto.id === id)));
      jest
        .spyOn(service.findResourcesService, "findAllByIdsWithPermissions")
        .mockImplementation(
          (ids) => new ResourcesCollection(resourcesDto.filter((resourceDto) => ids.includes(resourceDto.id))),
        );
      ...
      expect(service.findResourcesService.findAllByIdsWithPermissions).toHaveBeenCalledTimes(1);
      expect(service.findResourcesService.findAllByIdsWithPermissions).toHaveBeenCalledWith([
        resource1Dto.id,
        resource2Dto.id,
      ]);
      expect(service.findFoldersService.findAllByIdsWithPermissions).toHaveBeenCalledTimes(1);
      expect(service.findFoldersService.findAllByIdsWithPermissions).toHaveBeenCalledWith([
        parentFolder1Dto.id,
        parentFolder2Dto.id,
      ]);
      ...
      expect(service.shareResourceService.shareAll).toHaveBeenCalledWith(
        resourcesDto.map((resourceDto) => resourceDto.id),
        expectedResourcesPermissionChanges,
        "",
      );
    });
    ...
    it("moves and updates permissions with: owner resources, in owner or update folders, to owner folder", async () => {
      ...
      const destinationFolderPerms = [
        ...
        ...defaultPermissionsDtos(
          { aco: "Folder", aco_foreign_key: destinationFolderPerm.aco_foreign_key },
          { count: 2 },
        ),
      ];
  ...
  describe("notifies about its progress", () => {
    it("notifies of its progress when there is nothing to move (by instance when resources already in the destination folder).", async () => {
      ...
      expect(progressService.finishStep).toHaveBeenNthCalledWith(
        3,
        "Retrieving all resources parent folders permissions",
        true,
      );
    ...
    it("notifies of its progress when there are resources to move and to share.", async () => {
      ...
      expect(progressService.finishStep).toHaveBeenNthCalledWith(4, "Calculating resources permissions changes", true);
      expect(progressService.finishStep).toHaveBeenNthCalledWith(5, "Moving resources", true);
      expect(progressService._progress).toEqual(
        PROGRESS_STEPS_MOVE_RESOURCES_MOVE_ALL - PROGRESS_STEPS_SHARE_RESOURCES_SHARE_ALL,
      );
    });
  });
});
