...
var listen = function (worker) {
    ...
    worker.port.on('passbolt.setup.keyinfo.set', function(token, keydata) {
        try {
            ...
            worker.port.emit('passbolt.setup.keyinfo.set.complete', token, 'ERROR', error.message);
        }
    });
    ...
    worker.port.on('passbolt.setup.set', function(token, key, value) {
        try {
            ...
            worker.port.emit('passbolt.setup.set.complete', token, 'ERROR', error.message);
        }
    });
    ...
    worker.port.on('passbolt.setup.get', function(token, key) {
        try {
            ...
            worker.port.emit('passbolt.setup.get.complete', token, 'ERROR', error.message);
        }
    });
    ...
    worker.port.on('passbolt.setup.navigation.next', function(token, stepId) {
        try {
            ...
            worker.port.emit('passbolt.setup.navigation.next.complete', token, 'ERROR', error.message);
        }
    });
    ...
    worker.port.on('passbolt.setup.navigation.back', function(token) {
        try {
            ...
            worker.port.emit('passbolt.setup.navigation.back.complete', token, 'ERROR', error.message);
        }
    });
    ...
    worker.port.on('passbolt.setup.navigation.get.history', function(token) {
        try {
            ...
            worker.port.emit('passbolt.setup.navigation.get.history.complete', token, 'ERROR', error.message);
        }
    });
    ...
    worker.port.on('passbolt.setup.flush', function(token, key) {
        try {
            ...
            worker.port.emit('passbolt.setup.flush.complete', token, 'ERROR', error.message);
        }
    });
     ...
     */
    worker.port.on('passbolt.setup.checkKeyExistRemotely', function(token, fingerprint) {
        try {
            setup.checkKeyExistRemotely(fingerprint)
                .then(
                    function () {
                        worker.port.emit('passbolt.setup.checkKeyExistRemotely.complete', token, 'SUCCESS');
                    },
                    function (error) {
                        worker.port.emit('passbolt.setup.checkKeyExistRemotely.complete', token, 'ERROR', error.message);
                    }
              );
        }
        catch (error) {
    ...
    // The recovery has been completed, save the information
    worker.port.on('passbolt.setup.completeRecovery', function(token, data) {
        setup.completeRecovery(data)
            .then(
            function() {
                app.pageMods.passboltAuth.init(true);
                worker.port.emit('passbolt.setup.completeRecovery.complete', token, 'SUCCESS');
            },
            function(error) {
                worker.port.emit('passbolt.setup.completeRecovery.complete', token, 'ERROR', error);
            }
        );
    });
};
