...
describe("ShareResourceService", () => {
  ...
  describe("::shareAll", () => {
    each([
      {
        ...
        resourceTypeId: TEST_RESOURCE_TYPE_PASSWORD_STRING,
      },
      {
        title: "with password and description",
        secretClear: plaintextSecretPasswordAndDescriptionDto(),
        resourceTypeId: TEST_RESOURCE_TYPE_PASSWORD_AND_DESCRIPTION,
      },
      {
        title: "with password description and totp",
        secretClear: plaintextSecretPasswordDescriptionTotpDto(),
      ...
      it(`::${scenario.title}`, async () => {
        ...
        const permissionChanges = new PermissionChangesCollection([
          carolPermissionChange,
          bettyPermissionChange,
          adaPermissionChange,
        ]);

        // Mock request retrieving secrets that will need to be encrypted for new users through resources entry point.
        const resourceSecretData = await EncryptMessageService.encrypt(
          JSON.stringify(scenario.secretClear),
        ...
        const resourceDto = defaultResourceV4Dto({
          ...
          secrets: [
            plaintextSecretPasswordAndDescriptionDto({ resource_id: resourceIdToShare, data: resourceSecretData }),
          ],
        });
        jest
          .spyOn(service.findResourcesService, "findAllByIdsForShare")
        ...
        jest
          .spyOn(service.getOrFindResourcesService, "getOrFindByIds")
          .mockImplementation(() => new ResourcesCollection([resourceDto]));
        ...
        await expect(shareRequestData.secrets[0].data).toDecryptAndEqualTo(
    ...
    each([
      {
        ...
        secretClear: plaintextSecretPasswordStringDto(),
        resourceTypeId: TEST_RESOURCE_TYPE_V5_PASSWORD_STRING,
      },
      {
        title: "with password and description V5",
        secretClear: plaintextSecretPasswordAndDescriptionDto(),
        resourceTypeId: TEST_RESOURCE_TYPE_V5_DEFAULT,
      },
      {
        title: "with password description and totp V5",
        secretClear: plaintextSecretPasswordDescriptionTotpDto(),
        resourceTypeId: TEST_RESOURCE_TYPE_V5_DEFAULT_TOTP,
      },
      {
        title: "with standalone totp V5",
      ...
      it(`::${scenario.title}: with metadata personal to re-encrypt with shared key`, async () => {
        ...
        jest
          ...
          .mockImplementation(() => new MetadataKeysSettingsEntity(metadataKeysSettingsDto));
        ...
        const metadataKeysDto = [
          defaultMetadataKeyDto({
            id: metadataKeyId,
            armored_key: pgpKeys.metadataKey.public,
            metadata_private_keys: metadataPrivateKeysDto,
          }),
        ];
        jest
          .spyOn(service.encryptMetadataService.getOrFindMetadataKeysService, "getOrFindAll")
          .mockImplementation(() => new MetadataKeysCollection(metadataKeysDto));
        ...
        await expect(updateRequestResourceDto.metadata).toDecryptAndEqualTo(
          pgpKeys.metadataKey.private_decrypted,
          JSON.stringify(resourceDto.metadata),
        );
    ...
    it(`shares multiple resources in v4 and v5 having different metadata encryption requirements with multiple users `, async () => {
      ...
      const permissionChanges = new PermissionChangesCollection([
        permissionChangeR1V4Carol,
        permissionChangeR1V4Betty,
        permissionChangeR2V4Carol,
        permissionChangeR2V4Betty,
        permissionChangeR1V5Carol,
        permissionChangeR1V5Betty,
        permissionChangeR2V5Carol,
        permissionChangeR2V5Betty,
        permissionChangeR3V5Carol,
        permissionChangeR3V5Betty,
      ]);
      ...
      const resource1V4SecretData = await EncryptMessageService.encrypt(
        JSON.stringify(resource1V4SecretDataDto),
        await OpenpgpAssertion.readKeyOrFail(pgpKeys.admin.public),
      );
      const resource1V4Dto = defaultResourceV4Dto({
        ...
        secrets: [
          plaintextSecretPasswordAndDescriptionDto({ resource_id: resourceId1V4, data: resource1V4SecretData }),
        ],
      });
      const resource2V4SecretDataDto = plaintextSecretPasswordDescriptionTotpDto();
      const resource2V4SecretData = await EncryptMessageService.encrypt(
        JSON.stringify(resource2V4SecretDataDto),
        await OpenpgpAssertion.readKeyOrFail(pgpKeys.admin.public),
      );
      const resource2V4Dto = defaultResourceV4Dto({
        ...
        resource_type_id: TEST_RESOURCE_TYPE_PASSWORD_AND_DESCRIPTION,
        secrets: [
          plaintextSecretPasswordAndDescriptionDto({ resource_id: resourceId2V4, data: resource2V4SecretData }),
        ],
      });
      const resource1V5SecretDataDto = plaintextSecretPasswordDescriptionTotpDto();
      const resource1V5SecretData = await EncryptMessageService.encrypt(
        JSON.stringify(resource1V5SecretDataDto),
        await OpenpgpAssertion.readKeyOrFail(pgpKeys.admin.public),
      );
      const resource1V5Dto = defaultResourceDto({
        ...
        secrets: [
          plaintextSecretPasswordAndDescriptionDto({ resource_id: resourceId1V5, data: resource1V5SecretData }),
        ],
      });
      const resource2V5SecretDataDto = plaintextSecretTotpDto();
      const resource2V5SecretData = await EncryptMessageService.encrypt(
        JSON.stringify(resource2V5SecretDataDto),
        await OpenpgpAssertion.readKeyOrFail(pgpKeys.admin.public),
      );
      const resource2V5Dto = resourceStandaloneTotpDto({
        ...
        secrets: [
          plaintextSecretPasswordAndDescriptionDto({ resource_id: resourceId2V5, data: resource2V5SecretData }),
        ],
      });
      const resource3V5SecretDataDto = plaintextSecretPasswordStringDto();
      const resource3V5SecretData = await EncryptMessageService.encrypt(
        JSON.stringify(resource3V5SecretDataDto),
        await OpenpgpAssertion.readKeyOrFail(pgpKeys.admin.public),
      );
      const resource3V5Dto = defaultResourceDto({
        ...
        resource_type_id: TEST_RESOURCE_TYPE_V5_DEFAULT,
        secrets: [
          plaintextSecretPasswordAndDescriptionDto({ resource_id: resourceId3V5, data: resource3V5SecretData }),
        ],
      });
      ...
      jest
        .spyOn(service.findResourcesService, "findAllByIdsForShare")
        .mockImplementation(() => new ResourcesCollection(resourcesDto));
      ...
      let simulateR1V4RequestPermissions,
        simulateR2V4RequestPermissions,
        simulateR1V5RequestPermissions,
        simulateR2V5RequestPermissions,
      ...
      await expect(updateRequestResource1V5Dto.metadata).toDecryptAndEqualTo(
        pgpKeys.metadataKey.private_decrypted,
        JSON.stringify(resource1V5Dto.metadata),
      );
      ...
      await expect(updateRequestResource2V5Dto.metadata).toDecryptAndEqualTo(
        pgpKeys.metadataKey.private_decrypted,
        JSON.stringify(resource2V5Dto.metadata),
      );
