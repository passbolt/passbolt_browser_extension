...
var listen = function (worker) {
    ...
    worker.port.on('passbolt.context.dispatch', function (toWorker, name, value) {
        Worker.get(toWorker, workerContext).port.emit('passbolt.context.set', name, value);
    });

    // Dispatch an event to another worker.
    worker.port.on('passbolt.event.dispatch', function (toWorker, eventName) {
        var args = Array.slice(arguments, 1);
        if (Worker.exists(toWorker, workerContext)) {
            Worker.get(toWorker, workerContext).port.emit.apply(null, args);
        }
        else {
    ...
    var completedCallback = function (token) {
        ...
        if (callbacks[token]) {
            ...
            Worker.get(callbacks[token].toWorker, workerContext).port.removeListener(request + '.complete', callbacks[token].completedCallback);
            // Remove the listener on the progress event.
            Worker.get(callbacks[token].toWorker, workerContext).port.removeListener(request + '.progress', callbacks[token].progressCallback);
    ...
    worker.port.on('passbolt.request.dispatch', function (token, toWorker, request) {
        ...
        callbacks[token] = {
            ...
            completedCallback: function () {
                worker.port.emit.apply(null, ['passbolt.request.dispatch.complete'].concat(Array.slice(arguments, 1)));
            },
            progressCallback: function () {
                worker.port.emit.apply(null, ['passbolt.request.dispatch.progress'].concat(Array.slice(arguments, 1)));
            }
        };

        // Listen to the complete message on the worker we emit the request.
        Worker.get(toWorker, workerContext).port.on(request + '.complete', completedCallback);

        // Listen to the progress message on the worker we emit the request.
        Worker.get(toWorker, workerContext).port.on(request + '.progress', progressCallback);

        // Emit the message on the target worker.
        var args = [request, token].concat(Array.slice(arguments, 3));
        Worker.get(toWorker, workerContext).port.emit.apply(null, args);
    });
};
