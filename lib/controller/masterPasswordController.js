    ...
    var attempts = 0,
        ...
        _loop = function (masterPassword) {
            ...
            if (typeof masterPassword == 'undefined') {
                ...
                Worker.get('App', workerContext).port.emit('passbolt.keyring.master.request', token);
            }
            else {
                ...
                keyring.checkPassphrase(masterPassword).then(
                    function() {
                      // If everything went fine close the passphrase popup.
                        Worker.get('App', workerContext).port.emit('passbolt.keyring.master.request.close', token);
