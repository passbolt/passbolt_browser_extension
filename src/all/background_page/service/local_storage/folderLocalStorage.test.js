...
describe("FolderLocalStorage", () => {
  ...
  describe("::assertEntityBeforeSave", () => {
    it("Should throw if no data provided", async () => {
      expect.assertions(1);
      await expect(() => FolderLocalStorage.assertEntityBeforeSave()).toThrow(
        "FolderLocalStorage expects a FolderEntity to be set",
      );
    });

    it("Should throw if not a FolderEntity is provided", async () => {
      expect.assertions(1);
      await expect(() => FolderLocalStorage.assertEntityBeforeSave(42)).toThrow(
        "FolderLocalStorage expects an object of type FolderEntity",
      );
    });
    ...
    it("Should throw if the folder has no id", async () => {
      ...
      delete folderDto.id;
      const folder = new FolderEntity(folderDto);
      await expect(() => FolderLocalStorage.assertEntityBeforeSave(folder)).toThrow(
        "FolderLocalStorage expects FolderEntity id to be set",
      );
    });
    ...
    it("Should throw if the folder has no permission", async () => {
      ...
      const folder = new FolderEntity(folderDto);
      await expect(() => FolderLocalStorage.assertEntityBeforeSave(folder)).toThrow(
        "FolderLocalStorage::set expects FolderEntity permission to be set",
      );
    });
  });
