...
} from "passbolt-styleguide/src/shared/models/entity/metadata/metadataKeyEntity.test.data";
...
describe("SaveMetadataSettingsService", () => {
  ...
  beforeEach(async () => {
    ...
    jest
      .spyOn(saveMetadataSettingsService.shareMetadataKeyPrivateService.keyring, "sync")
  ...
  describe("::saveTypesSettings", () => {
    it("saves the metadata types settings to the API and store them into the local storage.", async () => {
      ...
      jest
        .spyOn(saveMetadataSettingsService.metadataTypesSettingsApiService, "save")
      ...
      expect(saveMetadataSettingsService.metadataTypesSettingsApiService.save).toHaveBeenCalledWith(
    ...
    it("saves the metadata types settings to the API and replace existing settings already stored in the local storage.", async () => {
      ...
      await saveMetadataSettingsService.metadataTypesSettingsLocalStorage.set(
        new MetadataTypesSettingsEntity(originalMetadataTypesSettingsDto),
      );
    ...
    it("throws if API return invalid settings.", async () => {
      ...
      jest.spyOn(saveMetadataSettingsService.metadataTypesSettingsApiService, "save").mockImplementation(() => {});

      await expect(() =>
        saveMetadataSettingsService.saveTypesSettings(metadataTypesSettings),
  ...
  describe("::saveKeysSettings", () => {
    it("saves the metadata keys settings to the API and store them into the local storage.", async () => {
      ...
      jest
        .spyOn(saveMetadataSettingsService.findMetadataSettingsService.metadataKeysSettingsApiService, "findSettings")
        .mockImplementation(() => defaultMetadataKeysSettingsDto());
      jest
        .spyOn(saveMetadataSettingsService.metadataKeysSettingsApiService, "save")
        .mockImplementation((settings) => settings);

      const savedSettings = await saveMetadataSettingsService.saveKeysSettings(
        metadataKeysSettings,
        pgpKeys.ada.passphrase,
      );

      expect(saveMetadataSettingsService.metadataKeysSettingsApiService.save).toHaveBeenCalledWith(
        metadataKeysSettings,
      );
    ...
    it("saves the metadata keys settings to the API and replace existing settings already stored in the local storage.", async () => {
      ...
      await saveMetadataSettingsService.metadataKeysSettingsLocalStorage.set(
        new MetadataKeysSettingsEntity(originalMetadataKeysSettingsDto),
      );
    ...
    it("should build metadata private keys for users having missing keys and for the server and sign the missing data keys signed by the current administrator", async () => {
      ...
      const metadataKeys = new MetadataKeysCollection([
        metadataKeyWithSignedMetadataPrivateKeyDataDto(),
        defaultMetadataKeyDto({ id: metadataKeyId, metadata_private_keys: [metadataPrivateKeyNotSigned] }),
      ]);
      ...
      jest
        .spyOn(saveMetadataSettingsService.findAndUpdateMetadataKeysSessionStorageService, "findAndUpdateAll")
        .mockImplementationOnce(() => metadataKeys);
      jest
        .spyOn(saveMetadataSettingsService.shareMetadataKeyPrivateService.getOrFindMetadataKeysService, "getOrFindAll")
        .mockImplementationOnce(() => metadataKeys);
      jest
        .spyOn(
          saveMetadataSettingsService.shareMetadataKeyPrivateService.findUsersService,
          "findAllActiveWithMissingKeys",
        )
        .mockImplementationOnce(() => usersCollection);
      jest
        .spyOn(saveMetadataSettingsService.shareMetadataKeyPrivateService.metadataPrivateKeyApiService, "create")
        .mockImplementation((metadataPrivateKeyCollection) => {
          for (const metadataPrivateKey of metadataPrivateKeyCollection) {
            const expectedMetadataPrivateKey = expectedMetadataPrivateKeys.find(
              (expectedMetadataPrivateKey) =>
                metadataPrivateKey.metadataKeyId === expectedMetadataPrivateKey.metadataKeyId,
            );
      ...
      jest.spyOn(
        saveMetadataSettingsService.shareMetadataKeyPrivateService.encryptMetadataPrivateKeysService,
        "encryptOne",
      );
      ...
      expect(
        saveMetadataSettingsService.shareMetadataKeyPrivateService.encryptMetadataPrivateKeysService.encryptOne,
      ).toHaveBeenCalledTimes(2);
      expect(
        saveMetadataSettingsService.shareMetadataKeyPrivateService.metadataPrivateKeyApiService.create,
      ).toHaveBeenCalledWith(
        new ShareMetadataPrivateKeysCollection([expectedMetadataPrivateKeys[0], expectedMetadataPrivateKeys[1]]),
      );
      expect(saveMetadataSettingsService.shareMetadataKeyPrivateService.shareAllMissing).toHaveBeenNthCalledWith(
        1,
        pgpKeys.ada.passphrase,
      );
      expect(settings.metadataPrivateKeys.hasDecryptedPrivateKeys()).toBeFalsy();
      expect(settings.metadataPrivateKeys.items[0].metadataKeyId).toStrictEqual(
        expectedMetadataPrivateKeys[2].metadataKeyId,
      );
      expect(settings.metadataPrivateKeys.items[0].userId).toStrictEqual(null);
      expect(settings.metadataPrivateKeys.items[1].metadataKeyId).toStrictEqual(
        expectedMetadataPrivateKeys[1].metadataKeyId,
      );
    ...
    it("throws if the given passphrase is not a string.", async () => {
      ...
      await expect(() =>
        saveMetadataSettingsService.saveKeysSettings(
          new MetadataKeysSettingsEntity(defaultMetadataKeysSettingsDto()),
          2,
        ),
    ...
    it("throws if API return invalid settings.", async () => {
      ...
      jest.spyOn(saveMetadataSettingsService.metadataKeysSettingsApiService, "save").mockImplementation(() => {});

      await expect(() =>
        saveMetadataSettingsService.saveKeysSettings(metadataKeysSettings, pgpKeys.ada.passphrase),
