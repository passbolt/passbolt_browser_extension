/**
 * Passbolt ~ Open source password manager for teams
 * Copyright (c) Passbolt SA (https://www.passbolt.com)
 *
 * Licensed under GNU Affero General Public License version 3 of the or any later version.
 * For full copyright and license information, please see the LICENSE.txt
 * Redistributions of files must retain the above copyright notice.
 *
 * @copyright     Copyright (c) Passbolt SA (https://www.passbolt.com)
 * @license       https://opensource.org/licenses/AGPL-3.0 AGPL License
 * @link          https://www.passbolt.com Passbolt(tm)
 * @since         3.4.0
 */
import Validator from 'validator';
import {EntitySchema} from "../abstract/entitySchema";
import {EntityValidationError} from "../abstract/entityValidationError";
import {AccountRecoveryOrganisationPolicyEntity} from "./accountRecoveryOrganisationPolicyEntity";

// Reset the modules before each test.
beforeEach(() => {
  window.Validator = Validator;
  jest.resetModules();
});

describe("AccountRecoveryOrganisationPolicy entity", () => {
  it("schema must validate", () => {
    EntitySchema.validateSchema(AccountRecoveryOrganisationPolicyEntity.ENTITY_NAME, AccountRecoveryOrganisationPolicyEntity.getSchema());
  });

  it("constructor works if valid DTO is provided", () => {
    const dto = {
      "id": "10801423-4151-42a4-99d1-86e66145a08c",
      "policy": "disabled",
      "created": "2020-05-04T20:31:45+00:00",
      "modified": "2020-05-04T20:31:45+00:00",
      "created_by": "d57c10f5-639d-5160-9c81-8a0c6c4ec856",
      "modified_by": "d57c10f5-639d-5160-9c81-8a0c6c4ec856",
      "account_recovery_organization_key_id": "45ce85c9-e301-4de2-8b41-298507002861",
      "account_recovery_organization_key": {
        "id": "d4c0e643-3967-443b-93b3-102d902c4510",
        "armored_key": "-----BEGIN PGP MESSAGE-----\r\nVersion: OpenPGP.js v4.6.2\r\nComment: https:\/\/openpgpjs.org\r\n\r\\r\n=AOt0\r\n-----END PGP MESSAGE-----\r\n",
        "fingerprint": "0C1D1761110D1E33C9006D1A5B1B332ED06426D3",
        "created": "2020-05-04T20:31:45+00:00",
        "modified": "2020-05-04T20:31:45+00:00",
        "created_by": "d57c10f5-639d-5160-9c81-8a0c6c4ec856",
        "modified_by": "d57c10f5-639d-5160-9c81-8a0c6c4ec856",
        "deleted": "2020-05-04T20:31:45+00:00"
      },
      "account_recovery_private_key_passwords": [{
        "id": "0dcde494-2231-43da-9bc5-6b39654b2a32",
        "recipient_foreign_model": "AccountRecoveryOrganisationKey",
        "recipient_foreign_key": "10801423-4151-42a4-99d1-86e66145a08c",
        "data": "-----BEGIN PGP MESSAGE-----\r\nVersion: OpenPGP.js v4.6.2\r\nComment: https:\/\/openpgpjs.org\r\n\r\\r\n=AOt0\r\n-----END PGP MESSAGE-----\r\n"
      }]
    };
    const entity = new AccountRecoveryOrganisationPolicyEntity(dto);
    expect(entity.toDto()).toEqual({
      "id": "10801423-4151-42a4-99d1-86e66145a08c",
      "policy": "disabled",
      "created": "2020-05-04T20:31:45+00:00",
      "modified": "2020-05-04T20:31:45+00:00",
      "created_by": "d57c10f5-639d-5160-9c81-8a0c6c4ec856",
      "modified_by": "d57c10f5-639d-5160-9c81-8a0c6c4ec856",
      "account_recovery_organization_key_id": "45ce85c9-e301-4de2-8b41-298507002861"
    });
    const contain = {account_recovery_organization_key: true, account_recovery_private_key_passwords: true};
    expect(entity.toDto(contain)).toEqual({
      "id": "10801423-4151-42a4-99d1-86e66145a08c",
      "policy": "disabled",
      "created": "2020-05-04T20:31:45+00:00",
      "modified": "2020-05-04T20:31:45+00:00",
      "created_by": "d57c10f5-639d-5160-9c81-8a0c6c4ec856",
      "modified_by": "d57c10f5-639d-5160-9c81-8a0c6c4ec856",
      "account_recovery_organization_key_id": "45ce85c9-e301-4de2-8b41-298507002861",
      "account_recovery_organization_key": {
        "id": "d4c0e643-3967-443b-93b3-102d902c4510",
        "armored_key": "-----BEGIN PGP MESSAGE-----\r\nVersion: OpenPGP.js v4.6.2\r\nComment: https:\/\/openpgpjs.org\r\n\r\\r\n=AOt0\r\n-----END PGP MESSAGE-----\r\n",
        "fingerprint": "0C1D1761110D1E33C9006D1A5B1B332ED06426D3",
        "created": "2020-05-04T20:31:45+00:00",
        "modified": "2020-05-04T20:31:45+00:00",
        "created_by": "d57c10f5-639d-5160-9c81-8a0c6c4ec856",
        "modified_by": "d57c10f5-639d-5160-9c81-8a0c6c4ec856",
        "deleted": "2020-05-04T20:31:45+00:00"
      },
      "account_recovery_private_key_passwords": [{
        "id": "0dcde494-2231-43da-9bc5-6b39654b2a32",
        "recipient_foreign_model": "AccountRecoveryOrganisationKey",
        "recipient_foreign_key": "10801423-4151-42a4-99d1-86e66145a08c",
        "data": "-----BEGIN PGP MESSAGE-----\r\nVersion: OpenPGP.js v4.6.2\r\nComment: https:\/\/openpgpjs.org\r\n\r\\r\n=AOt0\r\n-----END PGP MESSAGE-----\r\n"
      }]
    });
  });

  it("constructor returns validation error if dto required fields are missing", () => {
    try {
      new AccountRecoveryOrganisationPolicyEntity({});
    } catch (error) {
      expect(error instanceof EntityValidationError).toBe(true);
      expect(error.details).toEqual({
        policy: {required: 'The policy is required.'},
      });
    }
  });
});

