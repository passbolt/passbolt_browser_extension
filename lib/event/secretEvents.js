    ...
    worker.port.on('passbolt.secret.decrypt', function (token, txt) {
        ...
        masterPasswordController.get(token, worker)
            .then(function (masterPassword) {
                ...
                // Start loading bar.
                Worker.get('App', workerContext).port.emit('passbolt.event.trigger_to_page', 'passbolt_loading');
                ...
                crypto.decrypt(txt, masterPassword)
                    .then(
                        ...
                        function(decrypted) {
                            // Stop loading bar.
                            Worker.get('App', workerContext).port.emit('passbolt.event.trigger_to_page', 'passbolt_loading_complete');
                            // Complete process.
        ...
        keyring.sync()
            .then(function (keysCount) {
                ...
                if (keysCount) {
                    ...
                    Worker.get('App', workerContext).port.emit('passbolt.event.trigger_to_page', 'passbolt_notify', notification);
                }
