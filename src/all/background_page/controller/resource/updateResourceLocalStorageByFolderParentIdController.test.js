...
describe("UpdateResourceLocalStorageByFolderParentIdController", () => {
  ...
  describe("::exec", () => {
    it("should call for update the local storage given a folder ID", async () => {
      ...
      jest
        .spyOn(controller.findAndUpdateResourcesLocalStorage, "findAndUpdateAllByParentFolderId")
        .mockImplementation(() => {});
      ...
      expect(controller.findAndUpdateResourcesLocalStorage.findAndUpdateAllByParentFolderId).toHaveBeenCalledWith(
        parentFolderId,
      );
    });
    ...
    it("should call for update the local storage given a folder ID a second time with a passphrase if the first time it failed", async () => {
      ...
      jest
        .spyOn(controller.findAndUpdateResourcesLocalStorage, "findAndUpdateAllByParentFolderId")
        .mockImplementation(async (_, passphrase) => {
          if (!passphrase) {
