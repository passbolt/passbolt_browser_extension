...
/**
 * API client for the passwordless (passkey) endpoints. The ceremony "begin" and the WebAuthn call
...
class PasskeyApiService extends AbstractService {
  ...
  async beginLogin(userId) {
    ...
    const response = await this.apiClient.fetchAndHandleResponse("POST", url, this.apiClient.buildBody({ user_id: userId }));
    return response.body;
  }
  ...
  /**
   * List the current user's passwordless credentials (management UI).
   *
  ...
  /**
   * Delete one passwordless credential.
   *
