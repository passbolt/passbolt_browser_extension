...
describe("DecryptMessageService", () => {
  describe("DecryptMessageService::decryptSymmetrically", () => {
    ...
    it(
      "should decrypt a message and verify a signature",
      async () => {
        const messageClear = "message clear";
        const password = "bim bam boom";
        ...
        const messageEncryptedArmored = await EncryptMessageService.encryptSymmetrically(
          messageClear,
          [password],
          [signingKey],
        );
    ...
    it(
      "should decrypt a message and verify multiple signatures",
      async () => {
        const messageClear = "message clear";
        const password = "bim bam boom";
        ...
        const messageEncryptedArmored = await EncryptMessageService.encryptSymmetrically(
          messageClear,
          [password],
          [signingKeyUserA, signingKeyUserB],
        );
        const messageEncrypted = await OpenpgpAssertion.readMessageOrFail(messageEncryptedArmored);
        const resultDecrypt = await DecryptMessageService.decryptSymmetrically(messageEncrypted, password, [
          verifyingKeyA,
    ...
    it(
      "should throw an error if it cannot decrypt a message",
      async () => {
        const messageClear = "message clear";
        const password = "bim bam boom";
        const messageEncryptedArmored = await EncryptMessageService.encryptSymmetrically(messageClear, [password]);
    ...
    it(
      "should throw an error if it cannot verify a signature",
      async () => {
        const messageClear = "message clear";
        const password = "bim bam boom";
        const signingKey = await OpenpgpAssertion.readKeyOrFail(pgpKeys.ada.private_decrypted);
        ...
        const messageEncrypted = await OpenpgpAssertion.readMessageOrFail(messageEncryptedArmored);
        const resultDecrypt = DecryptMessageService.decryptSymmetrically(messageEncrypted, password, [
          verfificationKey,
        ]);
    ...
    it(
      "should throw an error if it cannot verify one of among multiple signatures",
      async () => {
        const messageClear = "message clear";
        const password = "bim bam boom";
        const signingKeyUserA = await OpenpgpAssertion.readKeyOrFail(pgpKeys.ada.private_decrypted);
  ...
  describe("DecryptMessageService::decrypt", () => {
    ...
    it(
      "should decrypt a message and return the raw result with options returnOnlyData to false",
      async () => {
        const messageClear = "message clear";
        const publicKey = await OpenpgpAssertion.readKeyOrFail(pgpKeys.betty.public);
        const privateKey = await OpenpgpAssertion.readKeyOrFail(pgpKeys.betty.private_decrypted);
        ...
        const messageEncrypted = await OpenpgpAssertion.readMessageOrFail(messageEncryptedArmored);
        const resultDecrypt = await DecryptMessageService.decrypt(
          messageEncrypted,
          privateKey,
          [verifyingKeyA, verifyingKeyB],
          {
    ...
    it(
      "should not throw an error if it cannot verify a signature and options throwOnInvalidSignaturesVerification is false",
      async () => {
        const messageClear = "message clear";
        const publicKey = await OpenpgpAssertion.readKeyOrFail(pgpKeys.betty.public);
        const privateKey = await OpenpgpAssertion.readKeyOrFail(pgpKeys.betty.private_decrypted);
    ...
    it(
      "should not throw an error if the message is signed but no verification key is passed while decrypting",
      async () => {
        const messageClear = "message clear";
        const publicKey = await OpenpgpAssertion.readKeyOrFail(pgpKeys.betty.public);
        const privateKey = await OpenpgpAssertion.readKeyOrFail(pgpKeys.betty.private_decrypted);
  ...
  describe("DecryptMessageService::decryptWithSessionKey", () => {
    it(
      "should decrypt a message with session key",
      async () => {
        const messageClear = "message clear";
        const publicKey = await OpenpgpAssertion.readKeyOrFail(pgpKeys.betty.public);
        const privateKey = await OpenpgpAssertion.readKeyOrFail(pgpKeys.betty.private_decrypted);
        ...
        sessionKey.algorithm = openpgp.enums.read(
          openpgp.enums.symmetric,
          messageEncrypted.packets[0].sessionKeyAlgorithm,
        );
    ...
    it(
      "should decrypt a message with session key and verify a signature",
      async () => {
        const messageClear = "message clear";
        const publicKey = await OpenpgpAssertion.readKeyOrFail(pgpKeys.betty.public);
        const privateKey = await OpenpgpAssertion.readKeyOrFail(pgpKeys.betty.private_decrypted);
    ...
    it(
      "should decrypt a message with session key and verify multiple signatures",
      async () => {
        const messageClear = "message clear";
        const publicKey = await OpenpgpAssertion.readKeyOrFail(pgpKeys.betty.public);
        const privateKey = await OpenpgpAssertion.readKeyOrFail(pgpKeys.betty.private_decrypted);
    ...
    it(
      "should throw an error if it cannot decrypt another message encrypted with session key",
      async () => {
        const messageClear = "message clear";
        const publicKey = await OpenpgpAssertion.readKeyOrFail(pgpKeys.betty.public);
        const privateKey = await OpenpgpAssertion.readKeyOrFail(pgpKeys.betty.private_decrypted);
