...
var getPublic = function(userId) {
  var txt = parseKeys(getFakeKey(), PUBLIC);
  var openpgpRes = openpgp.key.readArmored(txt);
  return openpgpRes.keys[0];
};
exports.getPublic = getPublic;

var getPrivate = function(userId) {
  var txt = parseKeys(getFakeKey(), PRIVATE);
  var openpgpRes = openpgp.key.readArmored(txt);
  return openpgpRes.keys[0];
};
...
var importPrivate = function (txt) {
  ...
  keyring.privateKeys.push(key);
  return keyring.privateKeys[keyring.privateKeys.length];
}
