...
var listen = function (worker) {
    ...
    worker.port.on('passbolt.user.get.remote', function (token) {
        user.getRemote()
            .then(
            function success(u) {
                worker.port.emit('passbolt.user.get.remote.complete', token, 'SUCCESS', u);
            },
    ...
    worker.port.on('passbolt.user.settings.set.domain', function (token, domain) {
        try {
            ...
            worker.port.emit('passbolt.user.settings.set.domain', token, 'SUCCESS');
        } catch (e) {
            worker.port.emit('passbolt.user.settings.set.domain', token, 'ERROR', e.message);
        }
    });
