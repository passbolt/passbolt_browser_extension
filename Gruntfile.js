  ...
  grunt.initConfig({
    ...
    shell: {
      ...
      build_safari_debug: {
        ...
        command: [
          './node_modules/.bin/crx pack ' + path.build + ' -p key.pem -o ' + path.dist_safari + 'passbolt-' + pkg.version + '-debug.crx',
          'rm -f ' + path.dist_safari + 'passbolt-latest@passbolt.com.crx',
          'ln -fs passbolt-' + pkg.version + '-debug.crx ' + path.dist_safari + 'passbolt-latest@passbolt.com.crx'
        ].join(' && ')
      },
      build_safari_prod: {
        options: {
        ...
        command: [
          ...
          './node_modules/.bin/crx pack ' + path.build + ' -p key.pem -o ' + path.dist_safari + 'passbolt-' + pkg.version + '.crx ',
          "echo '\nZip and Crx files generated in " + path.dist_safari + "'"
        ].join(' && ')
      }
    }
  });
