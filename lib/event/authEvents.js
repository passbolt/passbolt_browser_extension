...
var listen = function (worker) {
    ...
    worker.port.on('passbolt.auth.login', function (token, masterpassword) {
        Worker.get('Auth', worker).port.emit('passbolt.auth.login.start', token, 'SUCCESS', __('Logging in'));
        auth.login(masterpassword).then(
            function success(referrer) {
                ...
                Worker.get('Auth', worker).port.emit('passbolt.auth.login.complete', token, 'SUCCESS', msg, referrer);
            },
            function error(msg) {
                Worker.get('Auth', worker).port.emit('passbolt.auth.login.complete', token, 'ERROR', msg);
            }
        );
    });
