...
pageMod.PageMod({
  include: new RegExp(".*" + escapeRegExp(config.setupBootstrapUrl) + ".*"),
...
//include: /.*http:\/\/192\.168\.0\.108\/passbolt.*/,
  contentScriptWhen: 'ready',
  contentStyleFile: [
...
function listenSetupBootstrapEvents(worker) {
  worker.port.on('passbolt.setup.plugin_check', function(name, value) {
