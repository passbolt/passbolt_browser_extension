    ...
    worker.port.on('passbolt.secret.decrypt', function (token, txt) {
        ...
        masterPasswordController.get(token)
            .then(function (masterPassword) {
                ...
                // Start loading bar.
                Worker.get('App').port.emit('passbolt.event.trigger_to_page', 'passbolt_loading');
                ...
                crypto.decrypt(txt, masterPassword)
                    .then(
                        ...
                        function(decrypted) {
                            // Stop loading bar.
                            Worker.get('App').port.emit('passbolt.event.trigger_to_page', 'passbolt_loading_complete');
                            // Complete process.
        ...
        keyring.sync()
            .then(function (keysCount) {
                ...
                if (keysCount) {
                    ...
                    Worker.get('App').port.emit('passbolt.event.trigger_to_page', 'passbolt_notify', notification);
                }
