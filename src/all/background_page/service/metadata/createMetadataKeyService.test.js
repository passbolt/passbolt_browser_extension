...
describe("CreateMetadataKeyService", () => {
  ...
  describe("::create", () => {
    it(
      "creates, with disabled zero knowledge, a metadata key and shares it with all active users and the API keys.",
      async () => {
        ...
        jest
          .spyOn(service.getOrFindMetadataSettings, "getOrFindKeysSettings")
          .mockReturnValue(new MetadataKeysSettingsEntity(defaultMetadataKeysSettingsDto()));
        jest
          .spyOn(service.findUsersService, "findAllActive")
          .mockReturnValue(new UsersCollection([users.ada, users.betty]));
        ...
        const message1 = await OpenpgpAssertion.readMessageOrFail(
          metadataKeyToCreate.metadataPrivateKeys.items[0].data,
        );
        const decryptedData1 = JSON.parse(
          await DecryptMessageService.decrypt(message1, recipientPrivateKey1, [userPrivateKey]),
        );
        ...
        const message2 = await OpenpgpAssertion.readMessageOrFail(
          metadataKeyToCreate.metadataPrivateKeys.items[1].data,
        );
        const decryptedData2 = JSON.parse(
          await DecryptMessageService.decrypt(message2, recipientPrivateKey2, [userPrivateKey]),
        );
        ...
        const message3 = await OpenpgpAssertion.readMessageOrFail(
          metadataKeyToCreate.metadataPrivateKeys.items[2].data,
        );
        const decryptedData3 = JSON.parse(
          await DecryptMessageService.decrypt(message3, recipientPrivateKey3, [userPrivateKey]),
        );
    ...
    it(
      "create, with enabled zero knowledge, a metadata key and shares it with all active users only.",
      async () => {
        ...
        const metadataKeysSettings = new MetadataKeysSettingsEntity(
          defaultMetadataKeysSettingsDto({ zero_knowledge_key_share: true }),
        );
