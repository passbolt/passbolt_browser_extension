...
var listen = function (worker) {
    ...
    worker.port.on('passbolt.event.dispatch', function (toWorker, eventName) {
        ...
        if (Worker.exists(toWorker, worker.tab.id)) {
            Worker.get(toWorker, worker.tab.id).port.emit.apply(null, args);
        }
        else {
    ...
    var completedCallback = function (token) {
        ...
        if (callbacks[token]) {
            ...
            Worker.get(callbacks[token].toWorker, worker.tab.id).port.removeListener(request + '.complete', callbacks[token].completedCallback);
            // Remove the listener on the progress event.
            Worker.get(callbacks[token].toWorker, worker.tab.id).port.removeListener(request + '.progress', callbacks[token].progressCallback);
    ...
    worker.port.on('passbolt.request.dispatch', function (token, toWorker, request) {
        ...
        Worker.get(toWorker, worker.tab.id).port.on(request + '.complete', completedCallback);

        // Listen to the progress message on the worker we emit the request.
        Worker.get(toWorker, worker.tab.id).port.on(request + '.progress', progressCallback);
