...
pageMod.PageMod({
  ...
  contentScriptOptions: {
    ...
    templatePath: './tpl/keyring/master.ejs'
  },
  onAttach: listenMasterEvents
});

pageMod.PageMod({
  include: 'about:blank?passbolt=masterFailureInline*',
  //onAttach: onCsAttach,
  contentScriptFile: [
    data.url("js/lib/jquery-2.1.1.min.js"),
    data.url('js/lib/ejs_production.js'),
  ...
  contentScriptOptions: {
    expose_messaging: false,
    addonDataPath: data.url(),
    templatePath: './tpl/keyring/master_failure.ejs'
  },
  onAttach: listenMasterEvents
});
...
var includeCipherListeners = function (worker) {
  ...
  worker.port.on("passbolt.cipher.decrypt", function(token, txt) {
    ...
    // Try to decrypt.
    // If the private key hasn't been decrypted the Cipher controller will throw an exception.
    try {
      decrypted = CipherController.decrypt(worker, txt);
      worker.port.emit('passbolt.cipher.decrypt.complete', token, 'SUCCESS', decrypted);
    } catch (e) {
      // If the private key hasn't been decrypted, ask the user for the master password.
      callbacks[token] = function(token, masterPassword) {
        try {
          decrypted = CipherController.decrypt(worker, txt, masterPassword);
          worker.port.emit('passbolt.cipher.decrypt.complete', token, 'SUCCESS', decrypted);
        } catch (e) {
          workers['App'].port.emit('passbolt.keyring.master.request.complete', token, 'ERROR');
        }
      };
...
function listenMasterEvents(worker) {
  ...
  worker.port.on('passbolt.keyring.master.request.submit', function(token, masterPassword) {
    // Close the master password request dialog.
    workers['App'].port.emit('passbolt.dialog.close_latest');
