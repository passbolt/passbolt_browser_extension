    ...
    worker.port.on('passbolt.secret.decrypt', function (token, txt) {
        ...
        masterPasswordController.get(token)
            .then(function (masterPassword) {
                ...
                // Start loading bar.
                app.workers['App'].port.emit('passbolt.event.trigger_to_page', 'passbolt_loading');
                ...
                crypto.decrypt(txt, masterPassword)
                    .then(
                        ...
                        function(decrypted) {
                            // Stop loading bar.
                            app.workers['App'].port.emit('passbolt.event.trigger_to_page', 'passbolt_loading_complete');
                            // Complete process.
