...
describe("DecryptSecretsService", () => {
  ...
  describe("::decryptAllFromSecretRevisions", () => {
    it("should decrypt a collection of revisions", async () => {
      ...
      jest
        .spyOn(DecryptSecretsService, "getSecretEntityClassByResourceType")
        .mockReturnValue(SecretDataV5DefaultTotpEntity);
    ...
    it("should throw an error if something wrong happens during decryption and the errors are not ignored", async () => {
      ...
      jest.spyOn(DecryptMessageService, "decrypt").mockImplementation(() => {
        throw new Error("Impossible to decrypt message");
      });
      jest
        .spyOn(DecryptSecretsService, "getSecretEntityClassByResourceType")
        .mockReturnValue(SecretDataV5DefaultTotpEntity);
      ...
      await expect(() =>
        DecryptSecretsService.decryptAllFromSecretRevisions(
          secretRevisionCollection,
  ...
  describe("::getSecretEntityClassByResourceType", () => {
    it("should return the right secret data entity", async () => {
      ...
      expect(
        DecryptSecretsService.getSecretEntityClassByResourceType({ slug: RESOURCE_TYPE_PASSWORD_AND_DESCRIPTION_SLUG }),
      ).toBe(SecretDataV4DefaultEntity);
      expect(DecryptSecretsService.getSecretEntityClassByResourceType({ slug: RESOURCE_TYPE_TOTP_SLUG })).toBe(
        SecretDataV4StandaloneTotpEntity,
      );
      expect(
        DecryptSecretsService.getSecretEntityClassByResourceType({
          slug: RESOURCE_TYPE_PASSWORD_DESCRIPTION_TOTP_SLUG,
        }),
      ).toBe(SecretDataV4DefaultTotpEntity);
      expect(
        DecryptSecretsService.getSecretEntityClassByResourceType({ slug: RESOURCE_TYPE_V5_PASSWORD_STRING_SLUG }),
      ).toBe(SecretDataV5PasswordStringEntity);
      expect(DecryptSecretsService.getSecretEntityClassByResourceType({ slug: RESOURCE_TYPE_V5_DEFAULT_SLUG })).toBe(
        SecretDataV5DefaultEntity,
      );
      expect(
        DecryptSecretsService.getSecretEntityClassByResourceType({ slug: RESOURCE_TYPE_V5_DEFAULT_TOTP_SLUG }),
      ).toBe(SecretDataV5DefaultTotpEntity);
      expect(DecryptSecretsService.getSecretEntityClassByResourceType({ slug: RESOURCE_TYPE_V5_TOTP_SLUG })).toBe(
        SecretDataV5StandaloneTotpEntity,
      );
      expect(
        DecryptSecretsService.getSecretEntityClassByResourceType({ slug: RESOURCE_TYPE_V5_CUSTOM_FIELDS_SLUG }),
      ).toBe(SecretDataV5StandaloneCustomFieldsCollection);
      expect(
