 ...
 */
var Auth = function () {
    this.URL_VERIFY = '/auth/verify'; // @TODO get from server http headers
...
Auth.prototype.validateAuthToken = function(userAuthToken) {
    ...
    if (sections[0] !== section[3] && section[0] !== 'gpgauthv1.3.0') {
        throw new Error(__('Passbolt does not support this GPGAuth version'));
    }
    if (section[2] !== '36') {
        throw new Error(__('Passbolt does not support GPGAuth token nonce longer than 36 characters'));
    }
    if (!Validator.isUUID(section[1])) {
...
Auth.prototype.login = function(passphrase) {
    ...
    this._stage1(passphrase).then(
        function success(msg) {
            deferred.resolve(__('Stage1 ok!'));
            //this.stage2().then(
            //    function success() {
            //        deferred.resolve(__('Stage2 ok!'));
            //    },
            //    function failure() {
            //        deferred.reject(__('Stage2 fail!'));
            //    }
            //);
...
Auth.prototype._stage1 = function (passphrase) {
    ...
    Request({
        ...
        onComplete: function (raw) {
            ...
            var userAuthToken;
            try {
                var encryptedUserAuthToken = raw.headers['X-GPGAuth-User-Auth-Token'];
