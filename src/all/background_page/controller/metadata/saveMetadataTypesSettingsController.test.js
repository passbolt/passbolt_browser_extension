...
describe("SaveMetadataTypesController", () => {
  describe("::exec", () => {
    ...
    it("saves metadata types settings and update the local storage with it.", async () => {
      ...
      jest
        .spyOn(controller.saveMetadaSettingsService.metadataTypesSettingsApiService, "save")
      ...
      expect(controller.saveMetadaSettingsService.metadataTypesSettingsApiService.save).toHaveBeenCalledWith(
        new MetadataTypesSettingsEntity(metadataTypesSettingsDto),
      );
      expect(controller.saveMetadaSettingsService.metadataTypesSettingsLocalStorage.set).toHaveBeenCalledWith(
        new MetadataTypesSettingsEntity(metadataTypesSettingsDto),
      );
    });
    ...
    it("throws if the parameters are not valid.", async () => {
      expect.assertions(1);
      await expect(() => controller.exec("invalid metadata types settings entity", {})).toThrowEntityValidationError(
