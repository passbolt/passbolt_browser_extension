...
const mockedSyncImplementation = jest.fn();
const mockedKeyEntity = new ExternalGpgKeyEntity(mockedKeyInfo);
jest.mock('../../model/keyring', () => ({
  Keyring: jest.fn().mockImplementation(() => ({
    sync: mockedSyncImplementation,
    findPublic: jest.fn(userId => userId !== mockedUserId ? undefined : mockedKeyEntity)
  }))
}));
...
describe("GetUserKeyInfocontroller", () => {
  it(`Should return user key info from an existing userId`, async() => {
    ...
    const keyInfo = await controller.exec(mockedUserId);
    expect(keyInfo.toDto()).toStrictEqual(mockedKeyInfo);
  });
  ...
  it(`Should throw an exception if the given userId doesn't exist`, async() => {
    ...
    try {
      ...
      expect(mockedSyncImplementation).toHaveBeenCalledTimes(1);
    }
  });
});
