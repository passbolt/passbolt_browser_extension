...
var listen = function (worker) {
    ...
    worker.port.on('passbolt.context.dispatch', function (toWorker, name, value) {
        Worker.get(toWorker).port.emit('passbolt.context.set', name, value);
    });

    // Dispatch an event to another worker.
    worker.port.on('passbolt.event.dispatch', function (toWorker, eventName) {
        var args = Array.slice(arguments, 1);
        if (Worker.exists(toWorker)) {
            Worker.get(toWorker).port.emit.apply(null, args);
        }
		else {
    ...
    var completedCallback = function (token) {
        ...
        if (callbacks[token]) {
            ...
            Worker.get(callbacks[token].toWorker).port.removeListener(request + '.complete', callbacks[token].completedCallback);
            // Remove the listener on the progress event.
            Worker.get(callbacks[token].toWorker).port.removeListener(request + '.progress', callbacks[token].progressCallback);
    ...
    worker.port.on('passbolt.request.dispatch', function (toWorker, request, token) {
        ...
        Worker.get(toWorker).port.on(request + '.complete', completedCallback);

        // Listen to the progress message on the worker we emit the request.
        Worker.get(toWorker).port.on(request + '.progress', progressCallback);

        // Emit the message on the target worker.
        Worker.get(toWorker).port.emit.apply(null, Array.slice(arguments, 1));
    });
};
