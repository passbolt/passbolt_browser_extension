...
describe("AccountRecovery", () => {
  ...
  describe("AccountRecovery::attachEvents", () => {
    it("Should attach events", async () => {
      ...
      // expectations
      expect(GetRequestLocalAccountService.getAccountMatchingContinueUrl).toHaveBeenCalledWith(
        port._port.sender.tab.url,
      );
      expect(BuildApiClientOptionsService.buildFromAccount).toHaveBeenCalled();
      expect(AccountRecoveryEvents.listen).toHaveBeenCalledWith(
        { port: port, tab: port._port.sender.tab },
        undefined,
        undefined,
      );
    ...
    it("Should not attach events", async () => {
      ...
      jest.spyOn(GetRequestLocalAccountService, "getAccountMatchingContinueUrl").mockImplementation(() => {
