...
describe("ValidatePrivateGpgKeySetupController", () => {
  ...
  each([{ scenario: "dsa_3072", key: pgpKeys.dsa_3072.private }]).describe(
    ...
    (props) => {
      it(`should reject: ${props.scenario}`, async () => {
        expect.assertions(1);
        const controller = new ValidatePrivateGpgKeySetupController();
        await expect(controller.exec(props.key)).rejects.toStrictEqual(
          new Error("The private key should use a supported algorithm: RSA, ECDSA OR EDDSA."),
        );
      });
    },
  );
  ...
  it("Should throw an exception if the key is not formatted properly", async () => {
    ...
    const key = "Fake key";
    const controller = new ValidatePrivateGpgKeySetupController();
    await expect(controller.exec(key)).rejects.toStrictEqual(
      new Error("The key should be a valid openpgp armored key string."),
    );
  });
  ...
  it("Should throw an exception if the key is not a valid openpgp key", async () => {
    ...
    const controller = new ValidatePrivateGpgKeySetupController();
    await expect(controller.exec(key)).rejects.toStrictEqual(
      new Error("The private key should be a valid openpgp key."),
    );
  });
  ...
  it("Should throw an exception if the key is public", async () => {
    ...
    const controller = new ValidatePrivateGpgKeySetupController();
    await expect(controller.exec(key)).rejects.toStrictEqual(
      new Error("The key should be a valid openpgp private key."),
    );
  });
  ...
  it("Should throw an exception if the key has an expiration date", async () => {
    ...
    const controller = new ValidatePrivateGpgKeySetupController();
    await expect(controller.exec(key)).rejects.toStrictEqual(
      new Error("The private key should not have an expiry date."),
    );
  });
  ...
  each([
    ...
    it(`should reject: ${props.scenario}`, async () => {
      ...
      const controller = new ValidatePrivateGpgKeySetupController();
      await expect(controller.exec(props.key)).rejects.toStrictEqual(
        new Error("An RSA key should have a length of 3072 bits minimum."),
      );
    });
  });
  ...
  it(`Should throw if the private key is not an RSA and has no curve associated`, async () => {
    ...
    const controller = new ValidatePrivateGpgKeySetupController();
    await expect(controller.exec(dummyKey)).rejects.toStrictEqual(
      new Error("The private key should use a supported algorithm: RSA, ECDSA OR EDDSA."),
    );
  });
  ...
  it("Should throw if the private key is not an ECC with an unsupported curve", async () => {
    ...
    const controller = new ValidatePrivateGpgKeySetupController();
    await expect(controller.exec(dummyKey)).rejects.toStrictEqual(
      new Error("An ECC key should be based on a supported curve."),
    );
  });
});
