...
describe("EncryptMetadataPrivateKeysService", () => {
  ...
  describe("::encryptOne", () => {
    it(
      "encrypts a metadata private key for a user and mutate the metadata private key entity data with the encrypted result.",
      async () => {
        expect.assertions(3);
        ...
        const decryptedData = JSON.parse(
          await DecryptMessageService.decrypt(message, recipientPrivateKey, [userPrivateKey]),
        );
    ...
    it(
      "encrypts a metadata private key for the API and mutate the metadata private key entity data with the encrypted result.",
      async () => {
        expect.assertions(3);
    ...
    it(
      "encrypts a metadata private key and override the signature date.",
      async () => {
        expect.assertions(4);
        ...
        const recipientPrivateKey = await OpenpgpAssertion.readKeyOrFail(pgpKeys.ada.private_decrypted);
        const message = await OpenpgpAssertion.readMessageOrFail(metadataPrivateKey.data);
        const decryptResult = await DecryptMessageService.decrypt(message, recipientPrivateKey, [userPrivateKey], {
    ...
    it(
      "should encrypt a metadata private key and without sign it.",
      async () => {
        expect.assertions(4);
    ...
    it("throws if no key found for the user defined the metadata private key.", async () => {
      ...
      const expectedError = new TypeError(
        `The public key for the user with ID ${pgpKeys.ada.userId} could not be found.`,
      );
      const dto = decryptedMetadataPrivateKeyDto({ user_id: pgpKeys.ada.userId });
      const metadataPrivateKey = new MetadataPrivateKeyEntity(dto);
  ...
  describe("::encryptAll", () => {
    it(
      ...
      async () => {
        ...
        const dto2 = decryptedMetadataPrivateKeyDto({
          user_id: pgpKeys.betty.userId,
          metadata_key_id: dto1.metadata_key_id,
        });
  ...
  describe("::encryptAllFromMetadataKeyEntity", () => {
    it(
      "encrypts the metadata private keys from a metadata key entity and mutate all metadata private key entities with their encrypted result.",
      async () => {
        expect.assertions(7);
