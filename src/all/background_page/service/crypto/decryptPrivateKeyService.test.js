...
describe("DecryptPrivateKey service", () => {
  it(
    "should validate a private key with the right passphrase",
    async () => {
      expect.assertions(1);
  ...
  it(
    "should throw an InvalidMasterPasswordError when the passphrase is not correct",
    async () => {
      expect.assertions(1);
      const key = await OpenpgpAssertion.readKeyOrFail(pgpKeys.ada.private);
  ...
  it(
    "should throw an Error if the private key is already decrypted",
    async () => {
      expect.assertions(1);
      try {
  ...
  it(
    "should validate a private key armored and decrypt it",
    async () => {
      expect.assertions(1);
      await expect(
        DecryptPrivateKeyService.decryptArmoredKey(pgpKeys.ada.private, "ada@passbolt.com"),
