...
describe("ExpireMetadataKeyService", () => {
  ...
  describe("::expire", () => {
    it("expire the metadata key on the API.", async () => {
      ...
      jest
        .spyOn(service.getOrFindMetadataKeysService.findAndUpdateMetadataKeysService, "findAndUpdateAll")
        .mockImplementationOnce(() => new MetadataKeysCollection([metadataKeyDto]));
      ...
      expect(service.metadataKeysApiService.update).toHaveBeenCalledWith(
        expectedId,
        new MetadataKeyEntity({
          fingerprint: metadataKey.fingerprint,
          armored_key: publicKeyRevoked,
          expired: expiredDate,
        }),
      );
    });
