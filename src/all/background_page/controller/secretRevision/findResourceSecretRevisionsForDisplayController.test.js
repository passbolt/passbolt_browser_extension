...
describe("FindResourceSecretRevisionsForDisplayController", () => {
  describe("::exec", () => {
    it("should call the right services and return the right information", async () => {
      ...
      const expectedRevisionsCollection = new ResourceSecretRevisionsCollection(
        defaultResourceSecretRevisionsDtos({ resource_id }, { withCreator: true, withSecrets: true }),
      );
      ...
      jest
        .spyOn(controller.findAndDecryptSecretRevisionsService, "findAllByResourceIdAndDecryptForDisplay")
        .mockResolvedValue(expectedRevisionsCollection);
      ...
      expect(
        controller.findAndDecryptSecretRevisionsService.findAllByResourceIdAndDecryptForDisplay,
      ).toHaveBeenCalledTimes(1);
      expect(
        controller.findAndDecryptSecretRevisionsService.findAllByResourceIdAndDecryptForDisplay,
      ).toHaveBeenCalledWith(resource_id, pgpKeys.ada.passphrase);
    });
    ...
    it("should let user passphrase input cancelation error be thrown", async () => {
      ...
      jest.spyOn(controller.getPassphraseService, "getPassphrase").mockImplementation(() => {
        throw expectedError;
      });
