...
var Auth = function () {
    ...
    this.URL_PUBLIC_REGISTRATION_OPEN = '/auth/publicRegistrationOpen.json';
...
Auth.prototype.serverResponseHealthCheck = function(step, raw, deferred) {
    ...
    if(typeof raw.headers['X-GPGAuth-Error'] !== 'undefined') {
        ...
        // Handle specific case of non existing user, independant from the debug or non-debug mode.
        if (raw.headers['X-GPGAuth-Debug'] != undefined && raw.headers['X-GPGAuth-Debug'].indexOf('no user associated') != -1) {
            error_msg = raw.headers['X-GPGAuth-Debug'];
        }
    ...
    crypto.encrypt(
        ...
        .then(
            ...
            function(encrypted) {
                 ...
                 var url = user.settings.getDomain() + self.URL_VERIFY;
                 Request({
                     url: url,
                     content : data,
            ...
            function error(error) {
                deferred.reject(__('Unable to encrypt the verify token.'));
            }
        )
        // Exception
...
Auth.prototype.getServerKey = function(domain) {
    domain = typeof domain === 'undefined' ? user.settings.getDomain() : domain;
    var url = domain + this.URL_VERIFY,
        deferred = defer(),
        self = this;
    ...
    Request({
        ...
        onComplete: function (raw) {
            ...
            try {
                ...
                deferred.reject(__('There was a problem trying to understand the data provided by the server') +
                ' (Data:' + raw.text +')');
                return deferred.promise;
            }
...
Auth.prototype._stage1 = function (passphrase) {
    ...
    Request({
        ...
        onComplete: function (raw) {
            ...
            var crypto = new Crypto();
            crypto.decrypt(encryptedUserAuthToken, passphrase)
                .then(
                    // Success.
                    ...
                    // Error.
                    function(error) {
                        deferred.reject(  __('Unable to decrypt the key'));
                    }
                )
                // Exception.
                .catch(function(error) {
