...
/**
 * Validate a user, and return fields with errors in case of failure.
 *
 * @param user
 * @param fields
 */
...
/**
 * Tell if the current user is logged in.
 * @returns {*}
 */
 ...
 *
 * @param timeout
 *   How many seconds to store it.
 */
User.prototype.storeMasterPasswordTemporarily = function(masterPassword, timeout) {
    _masterPassword = {
 ...
 *
 * @param timeout
 *   timeout in seconds (example, if password should be deleted after 5 minutes, 5*60)
 *
 * @private
 */
User.prototype._loopDeleteMasterPasswordOnTimeout = function(timeout) {
    ...
    if (currentTimestamp >= _masterPassword.created + timeout) {
        ...
        setTimeout(function() {
          _this._loopDeleteMasterPasswordOnTimeout(timeout);
        }, 1000);
    }
};
