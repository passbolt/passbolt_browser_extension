...
(function (passbolt) {
   ...
   *
   * For each request a requestId is generated. This identifier will help to
   * retrieve the request associated callbacks and ensure the resolution of the
   * good callbacks once the request is completed.
   *
   * The requestId is added as parameter and should be sent back in the addon
   * code response.
   *
   ...
   *
   * ```
   * worker.port.emit('request_name.complete', requestId, 'ERROR' [, arg1, arg2 ..]);
   * ```
   *
   * 2.1 In case of success
   *
   * ```
   * worker.port.emit('request_name.complete', requestId, 'SUCCESS' [, arg1, arg2 ..]);
   * ```
   *
   * 2.1 Following the progress of a request
   *
   * ```
   * worker.port.emit('request_name.progress', requestId, 'PROGRESS' [, arg1, arg2 ..]);
   * ```
   *
  ...
  var _requestCompletedListener =  function(requestId, status) {
    ...
    if (status == 'SUCCESS') {
      ...
      _stack[requestId].deferred.rejectWith(this, args);
    }
    // @TODO Remove the progress listener.
    delete _stack[requestId];
  };
