...
describe("GenerateGpgKeyPair service", () => {
  it(
    "should generate a key pair according to the given parameters",
    async () => {
      ...
      expect(publicKeyInfo.userIds[0]).toEqual({
        name: generateGpgKeyPairOptionsDto.name,
        email: generateGpgKeyPairOptionsDto.email,
      });
      ...
      expect(privateKeyInfo.userIds[0]).toEqual({
        name: generateGpgKeyPairOptionsDto.name,
        email: generateGpgKeyPairOptionsDto.email,
      });
      ...
      const decryptedPrivateKey = await DecryptPrivateKeyService.decrypt(
        privateKey,
        generateGpgKeyPairOptionsDto.passphrase,
      );
  ...
  it(
    "should generate a key pair according to the given parameters even without creation date set",
    async () => {
      jest.useFakeTimers();
