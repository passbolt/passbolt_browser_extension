...
describe("EnableMetadataSetupSettingsController", () => {
  describe("::exec", () => {
    it("should call for the orchestrator to enable metadata", async () => {
      ...
      jest
        .spyOn(controller.configureMetadataSettingsService, "enableEncryptedMetadataForNewInstance")
        .mockImplementation(() => {});

      await controller.exec();

      expect(controller.getPassphraseService.getFromStorageOrFail).toHaveBeenCalledTimes(1);
      expect(controller.configureMetadataSettingsService.enableEncryptedMetadataForNewInstance).toHaveBeenCalledTimes(
        1,
      );
      expect(controller.configureMetadataSettingsService.enableEncryptedMetadataForNewInstance).toHaveBeenCalledWith(
        passphrase,
      );
    });
