...
var listen = function (worker) {
    ...
    worker.port.on('passbolt.keyring.public.find', function (token, userId) {
        var publicKey = keyring.findPublic(userId);
        if (typeof publicKey !== 'undefined') {
            worker.port.emit('passbolt.keyring.public.find.complete', token, 'SUCCESS', publicKey);
        } else {
            worker.port.emit('passbolt.keyring.public.find.complete', token, 'ERROR');
        }
    });

    // Send the private key information to the content code.
    worker.port.on('passbolt.keyring.private.find', function (token) {
        var info = keyring.findPrivate();
        if (typeof info !== 'undefined') {
            worker.port.emit('passbolt.keyring.private.find.complete', token, 'SUCCESS', info);
        } else {
            worker.port.emit('passbolt.keyring.private.find.complete', token, 'ERROR');
        }
    });
    ...
    worker.port.on('passbolt.keyring.private.backup', function (token, key) {
        ...
        promise.then(function () {
            worker.port.emit('passbolt.keyring.private.backup.complete', token, 'SUCCESS');
        }, function () {
            worker.port.emit('passbolt.keyring.private.backup.complete', token, 'ERROR');
        });
    });
