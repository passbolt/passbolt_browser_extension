...
describe("GetOrFindSessionKeysService", () => {
  ...
  describe("::getOrFindAllBundles", () => {
    it("with empty storage, retrieves the session keys bundles from the API and store them into the session storage.", async () => {
      ...
      jest
        .spyOn(
          getOrFindSessionKeysService.findAndUpdateSessionKeysService.findSessionKeysService
            .sesionKeysBundlesApiService,
          "findAll",
        )
      ...
      expectedSessionKeysBundlesDto.forEach(
        (sessionKeysBundleDto) =>
          (sessionKeysBundleDto.data = JSON.parse(pgpKeys.metadataKey.decryptedSessionKeysDataMessage)),
      );
    ...
    it("with populated storage, retrieves the session keys bundles from the session storage.", async () => {
      ...
      const sessionKeysBundlesCollectionDto = defaultSessionKeysBundlesDtos(
        {},
        { withDecryptedSessionKeysBundle: true },
      );

      await getOrFindSessionKeysService.sessionKeysBundlesSessionStorageService.set(
        new SessionKeysBundlesCollection(sessionKeysBundlesCollectionDto),
      );
      jest.spyOn(
        getOrFindSessionKeysService.findAndUpdateSessionKeysService.findSessionKeysService.sesionKeysBundlesApiService,
        "findAll",
      );
  ...
  describe("::getOrFindAllByForeignModelAndForeignIds", () => {
    it("with empty storage, retrieves the session keys bundles from the API and get all session keys by foreign model and foreign ids.", async () => {
      ...
      const collection = await getOrFindSessionKeysService.getOrFindAllByForeignModelAndForeignIds("Resource", [
    ...
    it("does not retrieve the passphrase from the session storage if passed as parameter", async () => {
      ...
      await getOrFindSessionKeysService.getOrFindAllByForeignModelAndForeignIds(
        "Resource",
        ["8e3874ae-4b40-590b-968a-418f704b9d9a"],
    ...
    it("with populated storage, retrieves the session keys bundles from the session storage and get all session keys by foreign model and foreign ids.", async () => {
      ...
      const expectedSessionKeysDto = [
        sessionKeysBundlesCollectionDto[3].data.session_keys[0],
        sessionKeysBundlesCollectionDto[1].data.session_keys[0],
      ];
