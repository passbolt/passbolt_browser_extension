...
/**
 * Check if the public registration is open.
 *
 * @param domain
 *   domain where to check. if domain is not provided, then look in the settings.
 *
 * @returns {d.promise|promise|jQuery.Deferred.promise|jQuery.promise|promise.promise}
 */
Auth.prototype.isPublicRegistrationOpen = function(domain) {
    var user = new User();
    domain = typeof domain === 'undefined' ? user.settings.getDomain() : domain;
    var url = domain + this.URL_PUBLIC_REGISTRATION_OPEN,
        deferred = defer(),
        self = this;
